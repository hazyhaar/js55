...
func TestClosureObjectStringQuotaAndRecovery(t *testing.T) {
	...
	if _, err := iso.Eval(ctx, `var closureText = "abcd";`); err != nil {
		t.Fatal(err)
	}
	...
	if _, err := iso.Eval(ctx, `new ArrayBuffer(1e12)`); !errors.Is(err, isolate.ErrMemoryLimitExceeded) {
		t.Fatalf("quota should still reject: %v", err)
	}
	v, err := iso.Eval(ctx, `var closureBox=Object(closureText); closureBox.answer=9; closureBox.answer;`)
